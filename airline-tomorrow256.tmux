...
main() {
  ...
  tmux set-option -g pane-active-border-fg "${dark_blue}"
  ...
  tmux set-option -g status-left-style "fg=${white},bg=${dark_blue},bold"
  tmux set-option -g status-right "${left_sep}#[bg=${black},reverse] %Y-%m-%d %H:%M "
  tmux set-option -g status-right-style "fg=${light_blue},bg=${dark_gray}"
  ...
  tmux set-window-option -g window-status-current-style "fg=${dark_gray},bg=${light_blue}"
}
